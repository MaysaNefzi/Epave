...
public class OffreController {
    ...
    @GetMapping("/offre_order")
    @CrossOrigin(origins = "http://localhost:4200")
    public Collection<Offre> offre_order(){
        return Offre_repo.getOffresoreder();
    }
    ...
    @DeleteMapping("/supprimer_offre/{id}")
    @CrossOrigin(origins = "http://localhost:4200")
    public Map<String, Boolean> supprimer_offre(@PathVariable Long id) {
        Offre o = null;
        try {
            o = Offre_repo.findById(id)
                    .orElseThrow(() -> new NotFoundException("Offre introuvable pour cet id: " + id));
